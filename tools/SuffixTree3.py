...
def get_str(args):
    if args.s:
        if len(args.s) == 1:
            ...
            print("Error! You can only specify two strings at most!\n")
            exit(1)
    else:
        string = ""
        if len(args.f) == 1:
...
class SuffixTree:
    ...
    def plotTree(self):
        temp_graph = """"""
        temp_graph+="digraph {\n"
        temp_graph+="\trankdir=LR\n"
        temp_graph+="\tedge [arrbufferwsize=0.4,fbufferntsize=10]\n"
        temp_graph+="\tnode1 [label=\"\","
        temp_graph+="style=filled,fillcbufferlbufferr=lightgrey,shape=circle,width=.1,height=.1]\n"
        # temp_graph+="//------leaves------\n"
        temp_graph = self.plotLeaves(self.root, temp_graph)
        # temp_graph += "//------internal nbufferdes------\n"
        temp_graph = self.plotInternalNodes(self.root, temp_graph)

        # temp_graph += "//------edges------\n"
        temp_graph = self.plotEdges(self.root, temp_graph)

        # temp_graph += "//------suffix links------\n"
        temp_graph = self.plotSLinks(self.root, temp_graph)
        temp_graph += "}\n"
    ...
    def plotLeaves(self, nodeID, dot_object):
        if len(nodeID.children) == 0:
            dot_object+="\tnode"
            dot_object+=str(nodeID.id)
            dot_object+=" [label=\"\",shape=point]\n"
        else:
            for child in nodeID.children.values():
                self.plotLeaves(child, dot_object)
    ...
    def plotInternalNodes(self, nodeID, dot_object):
        if (not nodeID == self.root) and len(nodeID.children) > 0:
            dot_object = dot_object + "\tnode" + str(nodeID.id) +" [label=\"\",style=filled,fillcolor=lightgrey,shape=circle,width=.07," +"height=.07]\n"
        for child in nodeID.children.values():
            self.plotInternalNodes(child, dot_object)
        return dot_object
    ...
    def plotEdges(self, nodeID, dot_object):
        for child in nodeID.children.values():
            dot_object = dot_object + "\tnode" + str(nodeID.id) + " -> node" + str(child.id) + " [label=\"" + self.edgeString(child) + "\",weight=3]\n"
            self.plotEdges(child, dot_object)
        return dot_object
    ...
    def plotSLinks(self, nodeID, dot_object):
        if nodeID.SuffixLink:
            dot_object = dot_object + "\tnode" + str(nodeID.id) + " -> node" + str(nodeID.SuffixLink.id) + " [label=\"\",weight=1,style=dotted]\n"
        for child in nodeID.children.values():
            self.plotSLinks(child, dot_object)
        return dot_object
